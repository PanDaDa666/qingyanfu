...
-(void)setCellFrame:(ChartCellFrame *)cellFrame
{
    ...
    [query getObjectInBackgroundWithId:chartMessage.icon block:^(AVObject *object, NSError *error) {
        if (!error) {
            AVFile *image = (AVFile *)[object objectForKey:@"HeadImage"];
            [self.icon sd_setImageWithURL:[NSURL URLWithString:[image getThumbnailURLWithScaleToFit:YES width:80 height:80]]];
        }else{
            NSLog(@"错误");
        }
    }];
    ...
    [self setBackGroundImageViewImage:self.chartView from:@"chatfrom_bg_normal.png" to:@"chatto_bg_normal.png"];
     self.chartView.contentLabel.text = nil;
     self.chartView.contentImageView.image = nil;
    if (chartMessage.messageMediaType==kAVIMMessageMediaTypeText) {
       ...
       {
        if (chartMessage.messageImageAVFile) {
            [self.chartView.contentImageView sd_setImageWithURL:[NSURL URLWithString:[chartMessage.messageImageAVFile getThumbnailURLWithScaleToFit:YES width:500 height:500]]];
        }else{
            self.chartView.contentImageView.image = chartMessage.messageImage;
        }
    }else if(chartMessage.messageMediaType ==kAVIMMessageMediaTypeAudio){
        self.chartView.playTimeLabel.text = [NSString stringWithFormat:@"%ld's",(long)chartMessage.playTime];
        NSLog(@"%ld",(long)chartMessage.playTime);
//        self.chartView.playAudioImageView =chartMessage.content;
    }
}
...
{
    UIImage *normal=nil ;
    if(!chartView.chartMessage.messageType){
        
        normal = [UIImage imageNamed:from];
        normal = [normal stretchableImageWithLeftCapWidth:normal.size.width * 0.5 topCapHeight:normal.size.height * 0.7];
        
    }else if(chartView.chartMessage.messageType){
...
-(void)copy:(id)sender
{
    if (self.contentStr) {
        ...
        [[UIPasteboard generalPasteboard]setString:@"啥都没有"];
    }
