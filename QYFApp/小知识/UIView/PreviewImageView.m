...
-(void)image:(UIImage *)image didFinishSavingWithError:(NSError *)error contextInfo:(void *)contextInfo
{
    if (error != NULL)
    {
        UIAlertView * alert=[[UIAlertView alloc]initWithTitle:@"o(╯□╰)o 保存失败" message:nil delegate:nil cancelButtonTitle:nil otherButtonTitles: nil];
    ...
    {
        UIAlertView * alert=[[UIAlertView alloc]initWithTitle:@"#^_^# 保存成功" message:nil delegate:nil cancelButtonTitle:nil otherButtonTitles: nil];
        [self performSelector:@selector(dimissAlert:) withObject:alert afterDelay:2.0f];
        [alert show];
        NSLog(@"save success!");
    }
